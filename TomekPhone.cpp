...
// He plans to release a new cellphone with an old-school keyboard, which may 
// require you to tap a key multiple times to type a single letter.
// For example, if the keyboard has two keys, one with the letters "adef" and the 
// other one with the letters "zyx", then typing 'a' requires one keystroke, 
// typing 'f' requires four keystrokes, typing 'y' requires two keystrokes, and 
// so on.
// 
// 
// 
// Tomek has already designed the keyboard's layout.
// That is, he already knows the number of keys on the keyboard, and for each key 
// he knows the maximum number of letters it may hold.
// He now wants to create a specific keyboard for a language that uses N 
// different letters.
// He has a large body of text in this language, and he already analyzed it to 
// find the frequencies of all N letters of its alphabet.
// 
// 
// 
// You are given a vector <int> frequencies with N elements.
// Each element of frequencies is the number of times one of the letters in 
// Tomek's alphabet appears in the text he has.
...
// needed to type Tomek's entire text.
// Return that minimum number of keystrokes.
// If there is not enough room on the keys and some letters of the alphabet won't 
// fit, return -1 instead.
// 
// 
// DEFINITION
// Class:TomekPhone
...
// Returns:int
// Method signature:int minKeystrokes(vector <int> frequencies, vector <int> 
// keySizes)
// 
// 
// CONSTRAINTS
...
// We can then type each A and each C using a single keystroke, and we need two 
// keystrokes for each B and each D.
// Therefore, the total number of keystrokes when typing the entire text will be 
// 7*1 + 3*2 + 4*1 + 1*2 = 19.
// 
// 
// 1)
