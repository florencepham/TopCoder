...
// In each round of the game, the cat who currently holds a ball flips a biased 
// coin. The coin will come up heads with probability p/1,000,000,000 and tails 
// with probability 1-(p/1,000,000,000).
// If the coin came up heads, the current cat will hand the ball to the next cat 
// clockwise, otherwise the current cat will hand the ball to the next cat 
// counterclockwise. Formally, if the current cat is cat j, heads means that the 
// ball goes to cat (j+1) mod N and tails means that it goes to cat (j-1) mod N.
// The game is played until each cat held the ball at least once. The cat who 
// holds the ball at the end of the game is the winner.
// 
// 
// 
// 
// In other words, the winner is the last cat to touch the ball.
// Note that cat 0 holds the ball at the beginning, and this does count as 
// holding the ball.
...
// We have p=30,000,000, hence the coin will come up heads with probability 
// 30,000,000/1,000,000,000 = 0.3 and tails with probability 0.7.
...
// At this moment, each cat has held the ball. The game ends and cat 1 gets the 
// prize.
// 
...
// Returns: 0.2
// 
// The coin that is flipped will come up heads with probability 1/2, and tails 
