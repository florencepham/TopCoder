// BEGIN CUT HERE
// PROBLEM STATEMENT
// Warning: This problem statement contains superscripts and/or subscripts. It 
// may not display properly outside of the applet.
// 
...
// conducted by the economists of Byteland have shown that for any m >= 2 the 
// revenue of a company that is created by simultainously merging m companies 
...
// mergers that joins all the companies. In each of the mergers, we may merge as 
// many of the currently existing companies as we wish.
// 
// DEFINITION
// Class:MergersDivOne
// Method:findMaximum
...
// Returns: 50.0
// 
// We may first merge companies 0 through 4 and then merge the resulting company 
// with company 5.
// 
// 4)
...
class MergersDivOne {
    ...
    {
        sort(revenues.begin(), revenues.begin() + revenues.size());
        
        double max = revenues[0]; 

        for (int i = 1; i < revenues.size(); i++) {
        	int revenue = revenues[i]; 
        	max = (max + revenue) / 2.0; 
        }
