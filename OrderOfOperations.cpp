...
class OrderOfOperations {
	...
	int minTime(vector <string> s) {
		...
		numOnes = countOnes(s, numOnes); 

		/* find the index of the string with minimum ones */
		int Time = 0; 
		int min = findMinOnes(numS, numOnes); 
		while (min != -1) {
			Time += (1 << min); 
			s = reduceOnes(s, numOnes, min); 
			numOnes = countOnes(s, numOnes); 
			min = findMinOnes(numOnes);
		}
	...
	private: 
	int findMinOnes(int numS, map<int, int> numOnes) {
		int min = 0;

		for (int i = 0; i < numS; i++) {
			if (numOnes[i] != 0 && 
			   (numOnes[min] == 0 || numOnes[i] < numOnes[min])) {
			   	min = i;
			} 
		}

		if (numOnes[min] == 0) return -1; 
		else return min; 
	}

	map<int, int> countOnes(vector <string> s, map<int, int> numOnes) {
		int lenS = s[0].length(); 
		int numS = s.size(); 
		for (int i = 0; i < numS; i++) {
	...
	vector<string> reduceOnes(vector <string> s, map<int, int> numOnes, int min) {
		int numS = s.size(); 
		for (int i = 0; i < numS; i++) {
			s[i] = s[i] ^ s[min]; 
		}
